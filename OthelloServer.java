package othelloSystem;




import java.io.BufferedReader;
import java.io.InputStreamReader;
import java.io.OutputStreamWriter;
import java.net.ServerSocket;
import java.net.Socket;

public class OthelloServer {

	ServerSocket serverSok = null; // 接続受け付け用ServerSocket
	Socket sok = null; // 接続用Socket
	OutputStreamWriter out; // 出力用ストリーム
	BufferedReader in; // 入力用ストリーム
	String from = null; // 相手から受けるデータの入力用
	String to = null; // 相手へ送るデータの入力用
	int port_number =15000; // ポートナンバー



	public OthelloServer() throws Exception{

		serverSok = new ServerSocket(port_number); // サーバソケットの作成

		System.out.println("Waiting for client connection\n");
		sok = serverSok.accept(); // クライアントからの接続待ち

		/* 準備：データ入力ストリームの定義--ソケットからデータを
		   取ってくる．sok -> in  */
		in = new BufferedReader
				(new InputStreamReader(sok.getInputStream(), "UTF8"));

		/* 準備：データ出力ストリームの定義--ソケットにデータを
		   書き込む．  sok <- out */
		out = new OutputStreamWriter(sok.getOutputStream(),"UTF8");



	}



	//サーバーにインデックスを送信するメソッド
	public int getData() throws Exception {

		from = in.readLine();

		return Integer.parseInt(from);

	}



	//サーバーからインデックスを受信するメソッド
	public void sendData(int data) throws Exception {

		to = data+""+"\n";
		out.write(to);
		out.flush();

	}



	public static void main(String[] args) {
		// TODO Auto-generated method stub

		OthelloServer server = null;
		int data;

		try{
			server = new OthelloServer();
		}catch(Exception e){
			System.out.println("OthelloServer:エラー1");
		}


		try{
			server.sendData(56);
		}catch(Exception e){
			System.out.println("OthelloServer:エラー2");
		}


		try{
			data = server.getData();
			System.out.println(data);
		}catch(Exception e){
			System.out.println("OthelloServer:エラー3");
		}







	}

}

